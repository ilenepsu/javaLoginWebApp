...
{   
    ...
    **/
    ...
    public void insert(Customer customer)
    {
        ...
        try {
            // REMOVE Customer_ID from the list of columns being inserted
            String sql = "INSERT INTO HD_Customer(Customer_First_Name, Customer_Last_Name, Customer_Favorite_Meal) VALUES (?, ?, ?)";

            // You might need to adjust how your DB wrapper (core.DB) executes this
            // to handle getting generated keys back. Assuming getPreparedStatement
            // can handle this:
            PreparedStatement stmt = db.getPreparedStatement(sql); // <-- Check your DB class if this needs extra parameters

            // Shift parameter indexes down by 1 since ID is no longer the first '?'
        ...
        // Make sure to close generatedKeys result set in a finally block
    }
    /**
